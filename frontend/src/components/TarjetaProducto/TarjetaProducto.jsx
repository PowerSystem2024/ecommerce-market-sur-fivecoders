...
export default function TarjetaProducto({ item, onView }) {
  ...
  return (
    <article
      className="w-full h-full bg-white rounded-xl shadow hover:shadow-lg transition flex flex-col p-4"
      role="group"
    ...
    >
      ...
      <div className="flex-1 flex flex-col justify-between">
        <header>
          <h3 id={`product-${item.id}`} className="text-base font-semibold text-orange-600 mb-1 truncate">
            {item.name}
          </h3>
          <p
            className="text-sm text-zinc-700 mb-2 line-clamp-2"
        ...
        <div className="mt-1 mb-3">
          <p className="text-xs text-zinc-500">Proveedor: {item.provider}</p>
        ...
        <footer className="pt-3 mt-auto border-t border-zinc-100">
          ...
          <div className="flex gap-2">
            ...
            {/* Antes había botón de favorito — eliminado */}
          </div>
        </footer>
      </div>
    </article>
